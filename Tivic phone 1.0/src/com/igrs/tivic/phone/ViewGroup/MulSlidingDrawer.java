...
public class MulSlidingDrawer extends ViewGroup {
	 ...
	 */
	public MulSlidingDrawer(Context context, AttributeSet attrs) {
	...
	public MulSlidingDrawer(Context context, AttributeSet attrs,
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.MulSlidingDrawer, defStyle, 0);

		int orientation = a.getInt(
				R.styleable.MulSlidingDrawer_direction,
				ORIENTATION_BTT);
		mVertical = (orientation == ORIENTATION_BTT || orientation == ORIENTATION_TTB);
		mBottomOffset = (int) a.getDimension(
				R.styleable.MulSlidingDrawer_bottomOffset, 0.0f);
		mTopOffset = (int) a.getDimension(
				R.styleable.MulSlidingDrawer_topOffset, 0.0f);
		mAllowSingleTap = a.getBoolean(
				R.styleable.MulSlidingDrawer_allowSingleTap, true);
		mAnimateOnClick = a.getBoolean(
				R.styleable.MulSlidingDrawer_animateOnClick, true);
		mInvert = (orientation == ORIENTATION_TTB || orientation == ORIENTATION_LTR);

		int handleId = a.getResourceId(
				R.styleable.MulSlidingDrawer_handle, 0);
		if (handleId == 0) {
		...
		int contentId = a.getResourceId(
				R.styleable.MulSlidingDrawer_content, 0);
		if (contentId == 0) {
			throw new IllegalArgumentException(
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mVertical) {
			int height = heightSpecSize - handle.getMeasuredHeight()
					- mTopOffset;
			mContent.measure(widthMeasureSpec,
